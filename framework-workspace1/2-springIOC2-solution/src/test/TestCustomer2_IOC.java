...
public class TestCustomer2_IOC {
	public static void main(String[] args) {
		...
		//현재 구조에서는 의존하는 컴포넌트를 변경하기 위해서는 사용하는 측의 코드 영역에 변화는 불가피하다
		//결합도가 높은 상태라고 표현한다
		
		//IOC 적용 -> 결합도를 느슨하게 한다
		//구체적인 항공사가 변경되어도 ( 컴포넌트(bean)가 변경되어도 사용하는 측에서는 변경 필요 x )
		//외부 영향이 최소화된다
		//1. 대상 컴포넌트들의 졔층구조를 형성 : 인터페이스 TicketService 추상메서드 ticketing();
		...
		// bean id 를 이용한 검색을 통해 bean(or component or 의존성)을 확보 -> DL ( Dependency Lookup )
