...
public class TestPerformanceCheck {
	 ...
	 * 서비스를 제공하는 각 메서드의 실행소요시간을 레벨별로 체크해서 리포트를 제공해야 한다
	 * 
	 * 리포트는 0.5초 이상 1초 미만 소요시에는 log.warn ( core class name 과 method name, 실행 소요시간 )
	 * 1초 초과시에는 log.error ( core class name 과 method name, 실행 소요시간 )
	 * 로 report.log 파일에 로깅하도록 한다
	 * 위와 같은 로그는 예외 발생 여부와 관계 없이 모두 로깅되어야 한다
	 * 
	 * 개발단계
	 * 1. 로그 : log4j -> 단위 테스트 2-SprintAOP2-ex2-log4j
	 * 2. 소요시간체크 : Spring Framework 에서 제공하는 StopWatch util class를 이용 -> 단위테스트 test.TestStopWatch
	 ...
	 * 	  -> org.kosta.aop.PerformanceReportService	timeLogging(ProceedingJoinPoint point)
	 * 	  2) AOP pointcut( 적용대상을 지정 ) -> bean(*Service) : Service로 끝나는 bean id 가 대상
