...
public class TestAOP {
	 ...
	 * - 2안) Component or Library 를 이용해 각 core에서는 import 해서 사용 -> 위의 첫번째 안보다는 효율적
	 * 	 Log4j Component를 이용해 개발 - 별도의 프로젝트에서 단위테스트( 2-springAOP2-ex2-log4j-test )
	 * 	 적용단계 1. maven dependency ( library ) Log4j
	 * 		   2. 설정 파일 log4j2.xml 추가
	 * 		   3. core의 대상 메서드에 로깅 기능을 적용 ( info level로 로깅 )
	 *  
	 *    횡단관심사( Cross Cutting Concern ) 인 로깅작업은 log4j 컴포넌트를 이용해
	 *    각 core에서는 import 후 logging 기능 코드 작업을 진행
	 *    -> 1안에 비해 작업을 많이 줄였지만 여전히 반복적인 작업은 불가피하다
	 *    	 타겟(core)이 많을 수록 부담은 증가한다.
	 *    
	 *    3안)AOP를 적용하면 기존 CORE 에 변경없이 별도의 모듈에서
	 *    	 횡단관심사( Cross Cutting Concern  )인 로깅 작업을 반영할 수 있다.
