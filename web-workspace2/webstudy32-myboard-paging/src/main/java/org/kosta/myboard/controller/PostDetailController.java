...
public class PostDetailController implements Controller {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//로그인 여부를 체크해서 로그인 상태가 아니면 홈으로 리다이렉트 시킨다
...
//		}
		...
		ArrayList<String>myboardNo = (ArrayList<String>) request.getSession().getAttribute("myboardNo");
		if(myboardNo.contains(no)==false) { //로그인 후 한번도 읽지않은 게시글이면 
			//조회수 증가 
