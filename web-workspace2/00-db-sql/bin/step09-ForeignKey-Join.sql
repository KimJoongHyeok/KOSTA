 ...
 * 
 * 	- Join SQL : 여러 테이블간의 정보를 결합하기 위한 SQL (기본적인 Join SQL 연습하고 상세한 것은 이후 공부예정)
 * 
 ...
 * 정규화를 거치면 아래와 같이 테이블을 분리하여 관계성을 가지면 된다	
 * 
 * 
 * 	부서 테이블 |---------O|<--- 사원테이블
 * 
 ...
 * 
 * 이때 테이블을 분리하여 관리할 때는 참조 무결성 보장을 위해 제약조건 Foreign Key 를 지정해야 한다.
 * 
 * 	
 * 
 */
...
-- foreignkey : constraint 제약조건명 foreignkey (컬럼명) references 참조테이블명(컬럼명)
-- 참조대상이 되는 테이블 (부서 departmetn)을 부모테이블이라고 하고 참조하는 테이블(사원 k_employee) 을 자식테이블이라고 한다.

-- 만약 부모테이블(부서 departmetn)에 10번 부서번호가 없다면 아래 insert구문은 error가 난다. - parent key not found
...
/*
 * JOIN SQL
 * - 사원번호 1 인 아이유 사원의 sal과 부서번호,부서명,지역을 조회하고자 한다
 * 사원테이블과 부서테이블의 정보를 조회해야함 -> join sql을 이용하면된다
 * 
 * 형식1
 * select 컬럼명,컬럼명
 * from 테이블명 별칭,테이블명 별칭
 * where 별칭.컬러명 = 별칭.컬럼명  -- 조인조건
 * 
 * 형식2
 * select 컬럼명,컬럼명
 * from 테이블명 별칭
