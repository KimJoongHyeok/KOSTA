...
실제 적용시 시퀀스 명.nextval : 다음 시퀀스 값이 반환
ex) 실제 DAO 적용사례
	insert into guestbook(guestbook_No,title) values (guestbook_seq.nextval , ?);
	
현재 시퀀스 값을 반환 -> 시퀀스명.currval
단 currval 은 시퀀스를 nextval 한 세션 내에서만 사용가능 (연결시작 ~ 연결종료시 까지 즉 nextval 한 컨넥션 내에서만 currval 이용할 수 있다)
( 참고 - sesstion : database 에 접속한 사용자가 접속 ~ 접속 종료시까지 유지되는 정보 ) 
...
--drop 한 test_seq 시퀀스명으로 다시 시퀀스를 생성해본다 (옵션ㅇ을 start wirth 7 적용해서 생성)
create sequence test_seq start with 7;

--car table 을 생성해서 sequence 를 이용해본다
create table car(
...
insert into car(car_no,model) values(car_seq.nextval,'테스라');
...
--시퀀스명.currval : 단독으로 사용될 수 없고 nextval 한 컨넥션 내에서 currval 을 사용할 수 있다. (동일한 세션내에서만 사용 가능)
