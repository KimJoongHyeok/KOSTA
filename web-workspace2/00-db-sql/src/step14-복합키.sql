...
insert into shares(customer_id,stock_id,quantity) values('spring','삼성',10);
insert into shares(customer_id,stock_id,quantity) values('java','LG',20);

-- 복합키의 제약조건 위배되므로 ERROR , customer_id와 stock_id 의 조합된 정보가 유일해야 하는 조건이라 error
insert into shares(customer_id,stock_id,quantity) values('java','삼성',50);
-- insert 가 아닌 update 를 해야함
