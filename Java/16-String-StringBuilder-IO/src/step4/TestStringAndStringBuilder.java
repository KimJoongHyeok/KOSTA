...
public class TestStringAndStringBuilder {
	public static void main(String[] args) {
		String name="아이유";//String 은 불변(상수) 
		System.out.println(name.concat("님"));//아이유님 문자열이 새로 생성 -> String 은 여러 곳에서 공유될 수 있으므로 
		System.out.println(name);//name은 아이유 ( 원본은 변하지 않았다 )
		System.out.println("****************");
		StringBuilder sb=new StringBuilder("아이유");//StringBuilder or StringBuffer 는 가변 
		System.out.println(sb.append("님"));//아이유님 -> StringBuidler는 자체가 변경  
		...
		System.out.println(nick.equals(snick.toString()));// equals로 비교됨 ( StringBuilder or StringBuffer를 toString() 필요 ) 
		System.out.println(nick.contentEquals(snick));// contentEquals는 String 과 StringBuilder or StringBuffer를 바로 비교 
	}
}
